...
namespace ADUser
{
    class Program
    {
        ...
        public static void GetUserInfo ( string namingContext, string username )
        {
            using ( DirectoryEntry de = new DirectoryEntry (
                    ...
                    ))
                    {
                        ...
                        if ( username == "all" )
                        { 
                            LDAPFilter = "(&(objectClass=user)(|(sAMAccountType=805306368)(sAMAccountType=805306370)))";
                        }
                        else
                        {
                            LDAPFilter = string.Format ( "(&(objectClass=user)(|(sAMAccountType=805306368)(sAMAccountType=805306370))(sAMAccountName={0}))", username );
                        }
        ...
        public static void Main ( string[] args )
        {
            ...
            {
                Console.WriteLine ( string.Format ( "\n[*] Usage: ADUser.exe [namingContext] [samaccountname | all]" ));
                System.Environment.Exit(0);
            }

            // get user information / data 
            GetUserInfo ( args[0].ToString(), args[1].ToString() );
        }
    }
}